...
const configPath = `${cwd}/.roadhogrc.api.doc.js`;
const config = fs.existsSync(configPath) ? require(configPath) : {};

module.exports = function() {
  const utilsDir = config.requestPath ? path.join(cwd, config.requestPath) : `${cwd}/src/utils`;
  ...
  const tplPath = config.tpl ? path.join(cwd, config.tpl) : path.join(__dirname, './static/tpl.js');
