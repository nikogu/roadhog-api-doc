...
`;
  ...
  "private": true,
  "scripts": {
    "build": "${isWin ? `node ./win-build.js` : `./node_modules/.bin/roadhog build`}"
  },
  "dependencies": {},
  "devDependencies": {}
}`;
  ...
  shelljs.exec('npm run build', function (code, stdout, stderr) {
    ...
    if (stderr && stderr.replace(/\s/ig,'') != 'null') {
      console.log(chalk.red('roadhog doc build failed.'));
    } else {
      console.log(chalk.green('roadhog doc build success.'));
    }
